...
{
    PathGraphicsItem *item_graphic = new PathGraphicsItem(point, this->model_->path_);
    this->canvas_->addItem(item_graphic);
...
PlaneModelItem *Controller::readPlane(QDataStream &in)
{
    ...
    PlaneModelItem *model = new PlaneModelItem(new QPointF(p1), new QPointF(p2));
    model->convex_ = convex;
...
void Controller::saveFile()
{
    ...
    if (file_name.isEmpty()) {
        ...
        if (!file.open(QIODevice::WriteOnly)) {
            QMessageBox::information(nullptr, QFileDialog::tr("Unable to open file"),
                file.errorString());
            return;
        }
