// TITLE:   Optimization_Interface/src/graphics/cylinder_graphics_item.cpp
...
CylinderGraphicsItem::CylinderGraphicsItem(CylinderModelItem *model,
    ...
    // Set resize handles
    this->width_handle_ =
            new CylinderResizeHandle(this->model_, this, 0);
    this->height_handle_ =
            new CylinderResizeHandle(this->model_, this, 1);
    this->trigger_width_handle_ =
            new CylinderResizeHandle(this->model_, this, 2);
    this->width_handle_->hide();
    this->height_handle_->hide();
    this->trigger_width_handle_->hide();
...
CylinderGraphicsItem::~CylinderGraphicsItem() {
    // delete handles
    delete this->width_handle_;
    delete this->height_handle_;
    delete this->trigger_width_handle_;
}

QRectF CylinderGraphicsItem::boundingRect() const {
    qreal height = this->model_->getHeight();
    qreal width = this->model_->getWidth();
    qreal trigger_width = this->model_->getTriggerWidth();
    // Add exterior border if direction flipped
    if (this->model_->getDirection()) {
        // scale with view
        width += trigger_width;
    }
    // return area of cylinder, QGraphicsItems stores rotation
    return QRectF(-width, -height, width* 2, height * 2);
}

void CylinderGraphicsItem::paint(QPainter *painter,
    ...
    if (this->isSelected()) {
        ...
        this->trigger_width_handle_->setPos(-width-trigger_width, 0);

        this->width_handle_->show();
        this->height_handle_->show();
        this->trigger_width_handle_->show();
    ...
    painter->drawRect(QRectF(-width, -height, width * 2, height * 2));

    // Draw clearance boundry
    this->clearance_pen_.setWidthF(3.0 / scaling_factor);
    qreal clearance_height = height;
    qreal clearance_width = width + trigger_width;
...
QPainterPath CylinderGraphicsItem::shape() const {
    // return shape of cylinder, QGraphicsItem handles rotation
    QPainterPath path;
    qreal height = this->model_->getHeight();
    qreal width = this->model_->getWidth();
    path.addRect(QRectF(-width, -height, width * 2, height * 2));
    return path;
}

void CylinderGraphicsItem::setRed(bool isOverlap) {
    // set color to red if overlapping
    if (isOverlap) {
        QColor fill = Qt::red;
        fill.setAlpha(200);
...
void CylinderGraphicsItem::flipDirection() {
...
QVariant CylinderGraphicsItem::itemChange(GraphicsItemChange change,
...
qreal CylinderGraphicsItem::getScalingFactor() const {
