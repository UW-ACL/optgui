// TITLE:   Optimization_Interface/src/controls/compute_worker.cpp
...
ComputeWorker::ComputeWorker(ConstraintModel *model) {
    this->model_ = model;
    this->initializeFly();
...
void ComputeWorker::initializeFly() {
...
void ComputeWorker::compute() {

    // Parameters
    // Number of points on the trajectory (resolution)
    this->fly_->P.K = std::max(std::min(
            this->model_->getHorizon(), MAX_HORIZON), 5);
    this->fly_->P.tf = this->model_->getFinaltime();   // duration of flight
    this->fly_->P.dt = this->fly_->P.tf
                              / (this->fly_->P.K - 1.);  // 'resolution'
    // Circle constraints | H(r - p) |^2 > R^2 where p is the center of the
    // circle and R is the radius (H some linear transform)
    this->fly_->P.obs.n = model_->
            loadEllipse(this->fly_->P.obs.R,
                        this->fly_->P.obs.c_e,
                        this->fly_->P.obs.c_n);
    // Affine constraints Ax leq b
    this->fly_->P.cpos.n = model_->
    ...
    uint32_t size = this->fly_->P.K;
    // GUI trajecotry points
    QVector<QPointF *> *trajectory = new QVector<QPointF *>(size);
    // Mikipilot trajectory to send to drone
    autogen::packet::traj3dof drone_traj3dof_data;
    drone_traj3dof_data.K = size;

    for (uint32_t i = 0; i < this->fly_->P.K; i++) {
        // Add points to GUI trajectory
        trajectory->append(new QPointF(this->fly_->O.r[2][i] * GRID_SIZE,
                           -this->fly_->O.r[1][i] * GRID_SIZE));
