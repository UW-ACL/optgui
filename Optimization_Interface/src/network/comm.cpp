// TITLE:   Optimization_Interface/src/network/comm.cpp
// AUTHORS: Ben Chasnov
...
comm::comm(const std::string& ip_address, uint16 port)
    : mc_ip_address(ip_address), mc_port(port) {
...
void comm::readPendingDatagrams() {
  while (mp_udp->hasPendingDatagrams()) {
    ...
    if ((bytes_read > 0)) {
      ...
      if (ptr_telemetry_data != ptr) {
          emit tx_pos(telemetry_data.pos_ned(0),
...
//    free(buffer);
//    buffer = nullptr;
    // this is the IP/port of drone
    mp_udp->writeDatagram(buff, ser_data.size(),
                          QHostAddress("192.168.1.101"), 6000);
//    mp_udp->writeDatagram(buff,ser_data.size(),
//                          QHostAddress("192.168.1.102"),6000);
//    mp_udp->writeDatagram(buff,ser_data.size(),
//                          QHostAddress("192.168.1.103"),6000);
//    mp_udp->writeDatagram(buff,ser_data.size(),
//                          QHostAddress("192.168.1.104"),6000);
    mp_udp->writeDatagram(buff, ser_data.size(),
                          QHostAddress("192.168.1.105"), 6000);
    mp_udp->writeDatagram(buff, ser_data.size(),
                          QHostAddress("192.168.1.106"), 6000);
    mp_udp->writeDatagram(buff, ser_data.size(),
                          QHostAddress("192.168.1.107"), 6000);
    mp_udp->writeDatagram(buff, ser_data.size(),
                          QHostAddress("192.168.1.108"), 6000);
    mp_udp->writeDatagram(buff, ser_data.size(),
                          QHostAddress("192.168.1.109"), 6000);
    mp_udp->writeDatagram(buff, ser_data.size(),
                          QHostAddress("192.168.1.110"), 6000);
}
